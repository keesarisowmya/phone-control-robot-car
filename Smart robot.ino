...
Serial.begin(9600); // this initializes the serial port with a baud
//rate of 9600.
...
char data = Serial.read(); // the incomming data from serial port is
//stored in the variable "data".
  Serial.print(data);  // to print the incomming data.
 if(data == 'f'){  // if the incomming data is "f" then excute the
//forward function
  forward();
 }
 if(data == 'b'){    // if the incomming data is "b" then excute the
//back function
  back();
 }
 if(data == 'r'){    // if the incomming data is "r" then excute the
//right function
  right();
 }
 if(data == 'l'){    //  if the incomming data is "l" then excute the
//left function
  left();
 }
 if(data == 's'){    // if the incomming data is "s" then excute the
//stop function
  Stop();
 }
